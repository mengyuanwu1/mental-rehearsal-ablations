...
const taskPhrase = (task: PriorityTask) =>
  `${task.durationMinutes} minutes on ${task.title.toLowerCase()}`;

const taskList = (scenario: Scenario) =>
  scenario.topTasks.map((task) => taskPhrase(task)).join(", then ");
...
export function scriptForCondition(scenario: Scenario, condition: ConditionId): string {
  const [first, second, third] = scenario.topTasks;

  switch (condition) {
    case "baseline":
      return [
        `Review today's preparation plan. Start with ${taskPhrase(first)}, then move to ${taskPhrase(second)}, and finish with ${taskPhrase(third)}.`,
    ...
    case "mind":
      return [
        ...
        `Picture rank 1 first: ${taskPhrase(first)}, the ${first.priority}-priority anchor. Then let rank 2 arrive as ${taskPhrase(second)}, followed by rank 3: ${taskPhrase(third)}.`,
    ...
    case "full":
      return [
        ...
        `Now move through the ranked sequence: ${taskPhrase(first)} for the most important stretch, ${taskPhrase(second)} for the next anchor, then ${taskPhrase(third)} to close the priority set. Let the longer block take more mental space, and let the shorter blocks feel lighter and cleaner.`,
