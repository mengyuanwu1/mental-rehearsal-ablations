...
for (let assignmentId = 0; assignmentId < slotCount; assignmentId += 1) {
  ...
  for (let trialIndex = 0; trialIndex < trialsPerSlot; trialIndex += 1) {
    const pairIndex = (offset + trialIndex) % pairs.length;
...
console.log({ pairCounts, scenarioCounts, pairScenarioBalanced, duplicateSlots });

if (!pairBalanced || !scenarioBalanced || !pairScenarioBalanced || !noDuplicates) {
  process.exitCode = 1;
}
